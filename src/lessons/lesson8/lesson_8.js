...
const str = "fgfggg";
console.group("8.1");
console.log("Первый элемен строки => ", str[0]);
console.groupEnd();
...
console.group("8.2");
const result2 = (2).plus(3).minus(1);
console.log(result2);
console.groupEnd();
...
console.group("8.3");
const result3 = joinStrings("*", "1", "b", "1c");
console.log(result3);
console.groupEnd();
//? Task 4
...
function getTopsRecursion(obj) {
  sum += obj.valueNode;
  if (obj.next) {
    obj.next.forEach((item) => {
      getTopsRecursion(item);
    });
  }
  return sum;
}
...
console.log(getTopsRecursion(tree));
...
//var change on let or wra on IIEF ()(i)
...
function Foo(Constr, bookName, bookAuthor) {
  return new Constr(bookName, bookAuthor);
}
...
const arr12 = [1, 2, 2, 3, 22, 4, 7, 22, 14, 12, 1, 9, 9, 0];
...
console.group("8.13");
console.log(doubledArr([1, 2, null, 7, 8, null, 3]));
console.groupEnd();
...
function getTreeValuesSum(obj) {
  result15 += obj.value;
  if (obj.children) {
    obj.children.forEach((item) => {
      getTreeValuesSum(item);
    });
  }
...
console.group("8.15");
console.log(getTreeValuesSum(tree2));
...
function missing(arr) {
  return [...arr].sort().reduce((acc, n, i, thisArr) => {
    if (thisArr[i] + 1 !== thisArr[i + 1] && thisArr[i + 1]) {
      acc = thisArr[i] + 1;
    }
...
console.group("%cCodewars", "color: white; background-color: red");
console.groupEnd("%cCodewars", "color: white; background-color: red");
